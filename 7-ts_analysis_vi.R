
library(feasts)
library(tsibble)
library(forecast)
library(stats)
library(zoo)
library(astsa)
library(urca) # for kpss test
library(TSA)
library(fable)
library(data.table)
library(tidyverse)
library(dplyr)
library(sf)
library(magrittr)
library(ggplot2)
library(scales)
library(lubridate)
library(BBmisc)
library(readr)

country <- 'Rwanda'

# Import the l8 and s1 timeseries 
l8_ts <- data.table(read_csv(paste0('output/time_series/', country, '_l8_ts.csv')))
s1_ts <- data.table(read_csv(paste0('output/time_series/', country, '_s1_ts.csv')))

# Import the plots data in include the plant date in the graph
plots <- data.table(read_csv(paste0('output/plot_data/', country, '/',country, '_plots_all.csv')))

# Filter out time stamps before the observation
l8_ts <- l8_ts[l8_ts$yearmon > as.Date('2013-01-01') & l8_ts$yearmon < as.Date('2022-11-01')] # start L8 observation
s1_ts <- s1_ts[s1_ts$yearmon > as.Date('2014-01-01') & s1_ts$yearmon < as.Date('2022-11-01')] # start S1 observation


############################# Create tsibble with the lagged variables 
#### L8
l8_ts_lag <- l8_ts %>% 
  group_by(yearmon) %>% 
  summarize(ndvi=mean(ndvi,na.rm=T),
            osavi=mean(osavi,na.rm=T),
            ndmi=mean(ndmi,na.rm=T),
            prcp=mean(prcp_month))

l8_ts_lag <- l8_ts_lag %>% 
  filter(yearmon >= min(l8_ts$date, na.rm=T)) %>% # Remove rows before first observation
  mutate(yearmonth=yearmonth(as.character(yearmon)),
         prcp_lag1=lag(prcp,1),
         prcp_lag2=lag(prcp,2),
         ndvi_int=na.approx(ndvi),
         osavi_int=na.approx(osavi),
         ndmi_int=na.approx(ndmi)) %>% # Fil gaps in time series
  as_tsibble(index=yearmonth)

l8_ref_lag <- l8_ts_lag %>% filter(year(yearmonth) < 2018)
l8_val_lag <- l8_ts_lag %>% filter(year(yearmonth) >= 2018)

#### S1
s1_ts_lag <- s1_ts %>% 
  group_by(yearmon) %>% 
  summarize(VH=mean(VH,na.rm=T),
            VV=mean(VV,na.rm=T),
            rvi=mean(rvi,na.rm=T),
            dbR=mean(dbR, na.rm=T),
            rfdi=mean(rfdi, na.rm=T),
            prcp=mean(prcp_month, na.rm=T))

s1_ts_lag <- s1_ts_lag %>% 
  # Remove rows before first observation
  filter(yearmon >= as.Date(paste0(year(min(s1_ts$date, na.rm=T)),'-', month(min(s1_ts$date, na.rm=T)),'-',1))) %>% 
  mutate(yearmonth=yearmonth(as.character(yearmon)),
         prcp_lag1=data.table::shift(prcp,n=1,type='lag'),
         prcp_lag2=data.table::shift(prcp,n=2,type='lag'),
         VH_int=na.approx(VH), # Fil gaps in time series
         rvi_int=na.approx(rvi),
         dbR_int=na.approx(dbR),
         rfdi_int=na.approx(rfdi)) %>% 
  as_tsibble(index=yearmonth)
s1_ts_lag <- s1_ts_lag[s1_ts_lag$yearmon>as.Date('2016-10-01'),]

s1_ref_lag <- s1_ts_lag %>% filter(year(yearmonth) < 2019)
s1_val_lag <- s1_ts_lag %>% filter(year(yearmonth) >= 2019)
########################### End creating tsibble


##################################### Fit and test full model
#### l8
# Fit arimax model and look at coeficients
l8_armax_mod <- l8_ref_lag %>% 
  model(arima = ARIMA(ndvi ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0))) %>% 
  report()
gg_tsresiduals(l8_armax_mod)

l8_armax_mod_osavi <- l8_ref_lag %>% 
  model(arima = ARIMA(osavi ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0))) %>% 
  report()
gg_tsresiduals(l8_armax_mod_osavi)

l8_armax_mod_ndmi <- l8_ref_lag %>% 
  model(arima = ARIMA(ndmi ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0))) %>% 
  report()
gg_tsresiduals(l8_armax_mod_ndmi)

# Plot full model
l8_fc_ts <- fabletools::forecast(l8_armax_mod, new_data = l8_val_lag[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2')])
l8_fc_ts_osavi <- fabletools::forecast(l8_armax_mod_osavi, new_data = l8_val_lag[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2')])
l8_fc_ts_ndmi <- fabletools::forecast(l8_armax_mod_ndmi, new_data = l8_val_lag[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2')])

l8_ts_ci <- l8_fc_ts$ndvi %>% 
  hilo(level = 95) 
l8_fc_ts <- l8_fc_ts %>% 
  mutate(upper_95=l8_ts_ci$upper,
         lower_95=l8_ts_ci$lower)

l8_ts_ci_osavi <- l8_fc_ts_osavi$osavi %>% 
  hilo(level = 95) 
l8_fc_ts_osavi <- l8_fc_ts_osavi %>% 
  mutate(upper_95=l8_ts_ci_osavi$upper,
         lower_95=l8_ts_ci_osavi$lower)

l8_ts_ci_ndmi <- l8_fc_ts_ndmi$ndmi %>% 
  hilo(level = 95) 
l8_fc_ts_ndmi <- l8_fc_ts_ndmi %>% 
  mutate(upper_95=l8_ts_ci_ndmi$upper,
         lower_95=l8_ts_ci_ndmi$lower)

ggplot()+
  geom_line(data=l8_ref_lag, 
            aes(x = yearmonth, y = ndvi_int - residuals(l8_armax_mod)$.resid), 
            col='darkred',lwd=0.9, alpha=0.8) +
  geom_line(data=l8_ts_lag, aes(x=yearmonth, y=ndvi_int), col='darkgreen',lwd=1)+
  geom_line(data=l8_fc_ts, aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.9, lty=1)+
  geom_ribbon(data=l8_fc_ts, aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red', lwd=0.8)+
  labs(y='NDVI', x='Time', title=paste('Actual (green) vs predicted (red) vegetation including 95% CI'))


ggplot()+
  geom_line(data=l8_ref_lag, 
            aes(x = yearmonth, y = ndvi_int - residuals(l8_armax_mod)$.resid), 
            col='darkred',lwd=0.9, alpha=0.8) +
  geom_line(data=l8_ts_lag, aes(x=yearmonth, y=ndvi_int), col='darkgreen',lwd=1)+
  geom_line(data=l8_fc_ts, aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.9, lty=1)+
  geom_ribbon(data=l8_fc_ts, aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red', lwd=0.8)+
  geom_line(data=l8_ref_lag, 
            aes(x = yearmonth, y = osavi_int - residuals(l8_armax_mod_osavi)$.resid), 
            col='orange',lwd=0.9, alpha=0.8) +
  geom_line(data=l8_ts_lag, aes(x=yearmonth, y=osavi_int), col='green',lwd=1)+
  geom_line(data=l8_fc_ts_osavi, aes(x = yearmonth, y = .mean), 
            col='orange',lwd=0.9, lty=1)+
  geom_ribbon(data=l8_fc_ts_osavi, aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='orange', lwd=0.8)+
  geom_line(data=l8_ref_lag, 
            aes(x = yearmonth, y = ndmi_int - residuals(l8_armax_mod_ndmi)$.resid), 
            col='blue',lwd=0.9, alpha=0.8) +
  geom_line(data=l8_ts_lag, aes(x=yearmonth, y=ndmi_int), col='darkblue',lwd=1)+
  geom_line(data=l8_fc_ts_ndmi, aes(x = yearmonth, y = .mean), 
            col='blue',lwd=0.9, lty=1)+
  geom_ribbon(data=l8_fc_ts_ndmi, aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='darkblue', lwd=0.8)+
  labs(y='NDVI', x='Time', title=paste('Actual (green) vs predicted (red) vegetation including 95% CI'))


#### S1
# Fit arimax model and look at coeficients
s1_armax_mod <- s1_ref_lag %>% 
  model(arima = ARIMA(rvi_int ~ prcp + prcp_lag1 + prcp_lag2, stepwise=T, ic='aic')) %>% 
  report()
gg_tsresiduals(s1_armax_mod)
s1_fc_ts <- fabletools::forecast(s1_armax_mod, new_data = s1_val_lag[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2')])

s1_ts_ci <- s1_fc_ts$rvi_int %>% 
  hilo(level = 95) 
s1_fc_ts <- s1_fc_ts %>% 
  mutate(upper_95=s1_ts_ci$upper,
         lower_95=s1_ts_ci$lower)


# Plot full model
ggplot()+
  geom_line(data=s1_ref_lag, 
            aes(x = yearmonth, y = rvi_int - residuals(s1_armax_mod)$.resid), 
            col='darkred',lwd=0.9, alpha=0.8) +
  geom_line(data=s1_ts_lag, aes(x=yearmonth, y=rvi_int), col='darkgreen',lwd=1)+
  geom_line(data=s1_fc_ts, aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.9, lty=1)+
  geom_ribbon(data=s1_fc_ts, aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red', lwd=0.8)+
  labs(y='RVI', x='Time', title=paste('Actual (green) vs predicted (red) vegetation including 95% CI'))




#################################################################### PER COUNTY
################### Aggregate per county and yearmon
#### L8
# Take mean ndvi and prcp
l8_ts_by_cnt <- l8_ts %>% 
  group_by(county, yearmon) %>% 
  summarize(ndvi=mean(ndvi, na.rm=T),
            osavi=mean(osavi,na.rm=T),
            ndmi=mean(ndmi,na.rm=T),
            prcp=mean(prcp_month)) 
# Lag the prcp
l8_ts_by_cnt <- l8_ts_by_cnt %>% 
  mutate(prcp_lag1=lag(prcp, 1),
         prcp_lag2=lag(prcp, 2))

# Identify the first observation date
l8_first_obs <- l8_ts_by_cnt %>%
  filter(!is.na(ndvi)) %>% 
  summarize(first_obs=min(yearmon),
            last_obs=max(yearmon))

# Exclude rows before 1st observation for each county
l8_ts_cnt <- l8_ts_by_cnt[l8_ts_by_cnt$county==l8_first_obs$county[1] & l8_ts_by_cnt$yearmon >= 
                            l8_first_obs$first_obs[1] & l8_ts_by_cnt$yearmon <= l8_first_obs$last_obs[1],]
for (i in 2:nrow(l8_first_obs)){
  print(i)
  plot_tsibble <- l8_ts_by_cnt[l8_ts_by_cnt$county==l8_first_obs$county[i] & l8_ts_by_cnt$yearmon >= 
                                 l8_first_obs$first_obs[i] & l8_ts_by_cnt$yearmon <= l8_first_obs$last_obs[i],]
  l8_ts_cnt <- rbind(l8_ts_cnt, plot_tsibble)
}

# Make it a tsibble and fill with lagged variables
l8_ts_cnt <- l8_ts_cnt %>% 
  mutate(yearmonth=yearmonth(as.character(yearmon)),
         ndvi_int=na.approx(ndvi),
         osavi_int=na.approx(osavi),
         ndmi_int=na.approx(ndmi)) %>% 
  as_tsibble(index=yearmonth, key=county)

# Create reference and validation datasets
l8_ref_cnt <- l8_ts_cnt %>% filter(year(yearmonth) < 2018)
l8_val_cnt <- l8_ts_cnt %>% filter(year(yearmonth) >= 2018)


#### S1
s1_ts_by_cnt <- s1_ts %>% 
  group_by(county, yearmon) %>% 
  summarize(VH=mean(VH, na.rm=T),
            VV=mean(VV,na.rm=T),
            rvi=mean(rvi, na.rm=T),
            rfdi=mean(rfdi, na.rm=T),
            prcp=mean(prcp_month,na.rm=T)) 

s1_ts_by_cnt <- s1_ts_by_cnt %>% 
  mutate(prcp_lag1=data.table::shift(prcp, n=1, type='lag'),
         prcp_lag2=data.table::shift(prcp, n=2, type='lag'))
s1_ts_by_cnt <- s1_ts_by_cnt[s1_ts_by_cnt$yearmon>as.Date('2016-10-01'),]

# Identify the first observation date
s1_first_obs <- s1_ts_by_cnt %>%
  filter(!is.na(VH)) %>% 
  summarize(first_obs=min(yearmon),
            last_obs=max(yearmon))

# Exclude rows before 1st observation for each county
s1_ts_cnt <- s1_ts_by_cnt[s1_ts_by_cnt$county==s1_first_obs$county[1] & s1_ts_by_cnt$yearmon >= 
                            s1_first_obs$first_obs[1] & s1_ts_by_cnt$yearmon <= s1_first_obs$last_obs[1],]
for (i in 2:nrow(s1_first_obs)){
  print(i)
  plot_tsibble <- s1_ts_by_cnt[s1_ts_by_cnt$county==s1_first_obs$county[i] & s1_ts_by_cnt$yearmon >= 
                                 s1_first_obs$first_obs[i] & s1_ts_by_cnt$yearmon <= s1_first_obs$last_obs[i],]
  s1_ts_cnt <- rbind(s1_ts_cnt, plot_tsibble)
}

# Make it a tsibble and fill with lagged variables
s1_ts_cnt <- s1_ts_cnt %>% 
  mutate(yearmonth=yearmonth(as.character(yearmon)),
         VH_int=na.approx(VH),
         VV_int=na.approx(VV),
         rvi_int=na.approx(rvi),
         rfdi_int=na.approx(rfdi)) %>% 
  as_tsibble(index=yearmonth, key=county)

# Create reference and validation datasets
s1_ref_cnt <- s1_ts_cnt %>% filter(year(yearmonth) < 2019)
s1_val_cnt <- s1_ts_cnt %>% filter(year(yearmonth) >= 2019)

##################################### End making COUNTY tsibbles


##################################### Forecast vegetation signal per COUNTY
#### L8
l8_armax_cnt <- l8_ref_cnt %>%
  model(ARIMA(ndvi_int ~ prcp + prcp_lag1 + prcp_lag2, stepwise=T, ic='aic'))
l8_fc_cnt <- fabletools::forecast(l8_armax_cnt, new_data = l8_val_cnt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'county')])
fabletools::forecast(l8_armax_cnt, new_data = l8_val_cnt[,c('yearmonth', 'prcp','prcp_lag1', 'prcp_lag2', 'county')]) %>%
  autoplot(l8_ts_cnt)+
  labs(x='Time', y='NDVI')

l8_armax_cnt_osavi <- l8_ref_cnt %>%
  model(ARIMA(osavi_int ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0)))
l8_fc_cnt <- fabletools::forecast(l8_armax_cnt_osavi, new_data = l8_val_cnt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'county')])
fabletools::forecast(l8_armax_cnt_osavi, new_data = l8_val_cnt[,c('yearmonth', 'prcp','prcp_lag1', 'prcp_lag2', 'county')]) %>%
  autoplot(l8_ts_cnt)+
  labs(x='Time', y='OSAVI')

l8_armax_cnt_ndmi <- l8_ref_cnt %>%
  model(ARIMA(ndmi_int ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0)))
l8_fc_cnt <- fabletools::forecast(l8_armax_cnt_ndmi, new_data = l8_val_cnt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'county')])
fabletools::forecast(l8_armax_cnt_ndmi, new_data = l8_val_cnt[,c('yearmonth', 'prcp','prcp_lag1', 'prcp_lag2', 'county')]) %>%
  autoplot(l8_ts_cnt)+
  labs(x='Time', y='NDMI')

# Extract confidence levels
l8_cnt_ci <- l8_fc_cnt$ndvi_int %>% 
  hilo(level = 95) 
l8_fc_cnt <- l8_fc_cnt %>% 
  mutate(upper_95=l8_cnt_ci$upper,
         lower_95=l8_cnt_ci$lower)

# Plot
cnt <- 'Bugesera'
ggplot()+
  geom_line(data=l8_ref_cnt[l8_ref_cnt$county==cnt,], 
            aes(x = yearmonth, y = ndvi - residuals(l8_armax_cnt[l8_armax_cnt$county==cnt,])$.resid), 
            col='darkred',lwd=0.65, alpha=0.6) +
  geom_line(data=l8_ts_cnt[l8_ts_cnt$county==cnt,], aes(x=yearmonth, y=ndvi_int), col='darkgreen',lwd=0.7)+
  geom_line(data=l8_ts_cnt[l8_ts_cnt$county==cnt,], 
            aes(x=yearmonth, y=normalize(prcp, method='range',c(0.1,0.2))), col='blue')+
  geom_line(data=l8_fc_cnt[l8_fc_cnt$county==cnt,], aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.65, lty=2)+
  geom_ribbon(data=l8_fc_cnt[l8_fc_cnt$county==cnt,], aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red')+
  labs(y='NDVI', x='Time', title=paste('Actual (green) vs predicted (red) NDVI in',cnt,'including 95% CI'))

#### S1
s1_armax_cnt <- s1_ref_cnt %>%
  model(ARIMA(rvi_int ~ prcp + prcp_lag1 + prcp_lag2, stepwise=T, ic='aic'))
s1_fc_cnt <- fabletools::forecast(s1_armax_cnt, new_data = s1_val_cnt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'county')])

fabletools::forecast(s1_armax_cnt, new_data = s1_val_cnt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'county')]) %>%
  autoplot(s1_ts_cnt)+
  labs(x='Time', y='rvi')

# Extract confidence levels
s1_cnt_ci <- s1_fc_cnt$rvi_int %>% 
  hilo(level = 95) 
s1_fc_cnt <- s1_fc_cnt %>% 
  mutate(upper_95=s1_cnt_ci$upper,
         lower_95=s1_cnt_ci$lower)

# Plot
cnt <- 'Nyagatare'
ggplot()+
  geom_line(data=s1_ref_cnt[s1_ref_cnt$county==cnt,], 
            aes(x = yearmonth, y = rvi_int - residuals(s1_armax_cnt[s1_armax_cnt$county==cnt,])$.resid), 
            col='darkred',lwd=0.65, alpha=0.6) +
  geom_line(data=s1_ts_cnt[s1_ts_cnt$county==cnt,], aes(x=yearmonth, y=rvi_int), col='darkgreen',lwd=0.7)+
  geom_line(data=s1_ts_cnt[s1_ts_cnt$county==cnt,], 
            aes(x=yearmonth, y=normalize(prcp, method='range',c(0.45,0.55))), col='blue')+
  geom_line(data=s1_fc_cnt[s1_fc_cnt$county==cnt,], aes(x = yearmonth, y = .mean),
            col='darkred',lwd=0.65,lty=2)+
  geom_ribbon(data=s1_fc_cnt[s1_fc_cnt$county==cnt,], aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red')+
  labs(y='rvi', x='Time', title=paste('Actual (green) vs predicted (red) VV backscatter in',cnt,'including 95% CI'))


s1_ts %>% 
  filter(plotID==) %>% 
  select(rvi, yearmon) %>% 
  group_by(yearmon) %>% 
  summarise(rvi=mean(rvi, na.rm=T)) %>% 
  ggplot()+
  geom_line(aes(x=yearmon, y=rvi))+
  geom_line(aes(x=yearmon, y=VV))+
  geom_line(aes(x=yearmon, y=VH))

############################################################### END PER COUNTY



############################################################### PER PLOT
################### Aggregate per plot and yearmonth
##### L8
l8_ts_by_plt <- l8_ts %>% 
  group_by(plotID, yearmon) %>% 
  summarize(ndvi=mean(ndvi, na.rm=T),
            osavi=mean(osavi,na.rm=T),
            ndmi=mean(ndmi,na.rm=T),
            prcp=mean(prcp_month)) 
# Make lagged variables
l8_ts_by_plt <- l8_ts_by_plt %>% 
  mutate(prcp_lag1=data.table::shift(prcp, n=1, type='lag'),
         prcp_lag2=data.table::shift(prcp, n=2, type='lag')) 

# Identify the first observation date 
l8_first_obs_plt <- l8_ts_by_plt %>%
  filter(!is.na(ndvi)) %>% 
  summarize(first_obs=min(yearmon),
            last_obs=max(yearmon))
# Exclude rows before 1st observation for each county
l8_ts_plt <- l8_ts_by_plt[l8_ts_by_plt$plotID==l8_first_obs_plt$plotID[1] & l8_ts_by_plt$yearmon >= 
                            l8_first_obs_plt$first_obs[1] & l8_ts_by_plt$yearmon <= l8_first_obs_plt$last_obs[1],]
for (i in 2:nrow(l8_first_obs_plt)){
  print(i)
  plot_tsibble <- l8_ts_by_plt[l8_ts_by_plt$plotID==l8_first_obs_plt$plotID[i] & l8_ts_by_plt$yearmon >= 
                                 l8_first_obs_plt$first_obs[i] & l8_ts_by_plt$yearmon <= l8_first_obs_plt$last_obs[i],]
  l8_ts_plt <- rbind(l8_ts_plt, plot_tsibble)
}

# Make it a tsibble and interpolate the NAs
l8_ts_plt <- l8_ts_plt %>% 
  mutate(ndvi_int=na.approx(ndvi),
         osavi_int=na.approx(osavi),
         ndmi_int=na.approx(ndmi))

fwrite(l8_ts_plt, paste0('output/time_series/', country, '_l8_plt.csv'))
l8_ts_plt <- data.table(read_csv(paste0('output/time_series/', country, '_l8_plt.csv')))

l8_ts_plt <- l8_ts_plt %>% 
  mutate(yearmonth=yearmonth(as.character(yearmon))) %>% 
  as_tsibble(index=yearmonth, key=plotID)

# Create reference and validation datasets
l8_ref_plt <- l8_ts_plt %>% filter(year(yearmonth) < 2018)
l8_val_plt <- l8_ts_plt %>% filter(year(yearmonth) >= 2018)



##### S1
s1_ts_by_plt <- s1_ts %>% 
  group_by(plotID, yearmon) %>% 
  summarize(VH=mean(VH, na.rm=T),
            VV=mean(VV, na.rm=T),
            rvi=mean(rvi,na.rm=T),
            rfdi=mean(rfdi,na.rm=T),
            prcp=mean(prcp_month)) 
# Make lagged variables
s1_ts_by_plt <- s1_ts_by_plt %>% 
  mutate(prcp_lag1=data.table::shift(prcp, n=1, type='lag'),
         prcp_lag2=data.table::shift(prcp, n=2, type='lag')) 
s1_ts_by_plt <- s1_ts_by_plt[s1_ts_by_plt$yearmon>as.Date('2016-10-01'),]

# Remote plots with more than 2 NA per plot
for (i in unique(s1_ts_by_plt$plotID[is.na(s1_ts_by_plt$VH)])){
  print(i)
  if (length(which(is.na(s1_ts_by_plt$VH[s1_ts_by_plt$plotID==i]))) > 2){
    s1_ts_by_plt <- s1_ts_by_plt[s1_ts_by_plt$plotID!=i,]
    print(paste(i, 'is deleted'))
  }
}

# Make it a tsibble and interpolate the NAs
s1_ts_plt <- s1_ts_by_plt %>% 
  mutate(VH_int=na.approx(VH),
         VV_int=na.approx(VV),
         rvi_int=na.approx(rvi),
         rfdi_int=na.approx(rfdi))

fwrite(s1_ts_plt, paste0('output/time_series/', country, '_s1_plt.csv'))
s1_ts_plt <- data.table(read_csv(paste0('output/time_series/', country, '_s1_plt.csv')))

# Make tsibble
s1_ts_plt <- s1_ts_plt %>% 
  mutate(yearmonth=yearmonth(as.character(yearmon))) %>% 
  as_tsibble(index=yearmonth, key=plotID)

# Create reference and validation datasets
s1_ref_plt <- s1_ts_plt %>% filter(year(yearmonth) < 2019)
s1_val_plt <- s1_ts_plt %>% filter(year(yearmonth) >= 2019)


####### OPTIONAL
# Identify the first observation date 
s1_first_obs_plt <- s1_ts_by_plt %>%
  filter(!is.na(VV)) %>% 
  summarize(first_obs=min(yearmon),
            last_obs=max(yearmon))
# Exclude rows before 1st observation for each county
s1_ts_plt <- s1_ts_by_plt[s1_ts_by_plt$plotID==s1_ts_by_plt$plotID[1] & s1_ts_by_plt$yearmon >= 
                            s1_first_obs_plt$first_obs[1] & s1_ts_by_plt$yearmon <= s1_first_obs_plt$last_obs[1],]
for (i in 2:nrow(s1_first_obs_plt)){
  print(i)
  plot_tsibble <- s1_ts_by_plt[s1_ts_by_plt$plotID==s1_first_obs_plt$plotID[i] & s1_ts_by_plt$yearmon >= 
                                 s1_first_obs_plt$first_obs[i] & s1_ts_by_plt$yearmon <= s1_first_obs_plt$last_obs[i],]
  s1_ts_plt <- rbind(s1_ts_plt, plot_tsibble)
}
######## END OPTIONAL 
################################## END AGGREGATION PER PLOT


##################################### Forecast vegetation signal per PLOT
#### L8
# NDVI
l8_armax_plt <- l8_ref_plt %>%
  model(ARIMA(ndvi_int ~ prcp + prcp_lag1 + prcp_lag2, stepwise=T))
l8_fc_plt <- fabletools::forecast(l8_armax_plt, new_data = l8_val_plt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'plotID')])

# Extract confidence levels
l8_plt_ci <- l8_fc_plt$ndvi_int %>% 
  hilo(level = 95) 
l8_fc_plt <- l8_fc_plt %>% 
  mutate(upper_95=l8_plt_ci$upper,
         lower_95=l8_plt_ci$lower)
save(l8_fc_plt, file = paste0('output/models/', country, '_l8_fc_plt_ndvi.RDS'))
save(l8_armax_plt, file = paste0('output/models/', country, '_l8_armax_plt_ndvi.RDS'))

# OSAVI
l8_armax_plt_osavi <- l8_ref_plt %>%
  model(ARIMA(osavi_int ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0)))
l8_fc_plt_osavi <- fabletools::forecast(l8_armax_plt_osavi, new_data = l8_val_plt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'plotID')])

# Extract confidence levels
l8_plt_ci_osavi <- l8_fc_plt_osavi$osavi_int %>% 
  hilo(level = 95) 
l8_fc_plt_osavi <- l8_fc_plt_osavi %>% 
  mutate(upper_95=l8_plt_ci_osavi$upper,
         lower_95=l8_plt_ci_osavi$lower)
save(l8_fc_plt_osavi, file = paste0('output/models/', country, '_l8_fc_plt_osavi.RDS'))
save(l8_armax_plt_osavi, file = paste0('output/models/', country, '_l8_armax_plt_osavi.RDS'))

# NDMI
l8_armax_plt_ndmi <- l8_ref_plt %>%
  model(ARIMA(ndmi_int ~ prcp + prcp_lag1 + prcp_lag2 + pdq(1, 0, 1) + PDQ(1, 0, 0)))
l8_fc_plt_ndmi <- fabletools::forecast(l8_armax_plt_ndmi, new_data = l8_val_plt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'plotID')])

# Extract confidence levels
l8_plt_ci_ndmi <- l8_fc_plt_ndmi$ndmi_int %>% 
  hilo(level = 95) 
l8_fc_plt_ndmi <- l8_fc_plt_ndmi %>% 
  mutate(upper_95=l8_plt_ci_ndmi$upper,
         lower_95=l8_plt_ci_ndmi$lower)
save(l8_fc_plt_ndmi, file = paste0('output/models/', country, '_l8_fc_plt_ndmi.RDS'))
save(l8_armax_plt_ndmi, file = paste0('output/models/', country, '_l8_armax_plt_ndmi.RDS'))


load(paste0('output/models/', country, '_l8_plt_fc.RDS'))
load(paste0('output/models/', country, '_l8_armax_plt.RDS'))

components(l8_fc_plt)
accuracy(l8_fc_plt, l8_ts_plt)

# Plot
plt <- 5962
ggplot()+
  #geom_line(data=l8_ref_plt[l8_ref_plt$plotID==plt,], 
            #aes(x = yearmonth, y = ndvi_int - residuals(l8_armax_plt[l8_armax_plt$plotID==plt,])$.resid), 
            #col='darkred',lwd=0.65, alpha=0.6) +
  geom_line(data=l8_ts_plt[l8_ts_plt$plotID==plt,], aes(x=yearmonth, y=ndvi_int), col='darkgreen',lwd=0.7)+
  geom_line(data=l8_ts_plt[l8_ts_plt$plotID==plt,], 
            aes(x=yearmonth, y=normalize(prcp, method='range',c(0.2,0.3))), col='blue')+
  geom_line(data=l8_fc_plt[l8_fc_plt$plotID==plt,], aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.65, lty=2)+
  geom_ribbon(data=l8_fc_plt[l8_fc_plt$plotID==plt,], aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red')+
  geom_point(data = l8_ts[l8_ts$plotID==plt,], aes(x = yearmon, y = ndvi), alpha=0.5)+
  geom_vline(xintercept=plots[plots$plotID==plt,]$plant_date,lty=3,alpha=0.75)+
  labs(y='NDVI', x='Time', title=paste('Actual (green) vs predicted (red) NDVI in including 95% CI'))


#### S1
s1_armax_plt <- s1_ref_plt %>%
  model(ARIMA(rvi_int ~ prcp + prcp_lag1 + prcp_lag2, stepwise=T))
s1_fc_plt <- fabletools::forecast(s1_armax_plt, new_data = s1_val_plt[,c('yearmonth', 'prcp', 'prcp_lag1', 'prcp_lag2', 'plotID')])

# Extract confidence levels
s1_plt_ci <- s1_fc_plt$rvi_int %>% 
  hilo(level = 95) 
s1_fc_plt <- s1_fc_plt %>% 
  mutate(upper_95=s1_plt_ci$upper,
         lower_95=s1_plt_ci$lower)

save(s1_fc_plt, file = paste0('output/models/', country, '_s1_fc_plt_rvi.RDS'))
save(s1_armax_plt, file = paste0('output/models/', country, '_s1_armax_plt_rvi.RDS'))
load(paste0('output/models/', country, '_s1_fc_plt_rvi.RDS'))
load(paste0('output/models/', country, '_s1_armax_plt_rvi.RDS'))

accuracy(s1_fc_plt, s1_ts_plt)

# Plot
plt <- 17552
ggplot()+
  #geom_line(data=s1_ref_plt[s1_ref_plt$plotID==plt,], 
  #          aes(x = yearmonth, y = rvi_int - residuals(s1_armax_plt[s1_armax_plt$plotID==plt,])$.resid), 
  #          col='darkred',lwd=0.65, alpha=0.6) +
  geom_line(data=s1_ts_plt[s1_ts_plt$plotID==plt,], aes(x=yearmonth, y=rvi_int), col='darkgreen',lwd=0.7)+
  geom_line(data=s1_ts_plt[s1_ts_plt$plotID==plt,], 
            aes(x=yearmonth, y=normalize(prcp, method='range',c(0.4,0.45))), col='blue')+
  geom_line(data=s1_fc_plt[s1_fc_plt$plotID==plt,], aes(x = yearmonth, y = .mean), 
            col='darkred',lwd=0.65, lty=2)+
  geom_ribbon(data=s1_fc_plt[s1_fc_plt$plotID==plt,], aes(x=yearmonth, ymin=lower_95, ymax=upper_95),
              alpha=0.1, fill='red')+
  #geom_point(data = s1_ts[s1_ts$plotID==plt,], aes(x = yearmon, y = rvi), alpha=0.5)+
  geom_vline(xintercept=plots[plots$plotID==plt,]$plant_date,lty=3,alpha=0.75)+
  labs(y='RVI', x='Time', title=paste('Actual (green) vs predicted (red) VV in including 95% CI'))
